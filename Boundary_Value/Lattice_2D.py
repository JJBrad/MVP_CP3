...
def plotLat(lat, xCross, yCross, interp, norm, fileName):
...
class lattice(object):
    ...
    """
    def __init__(self, xDim=50, yDim=0, dx=1., dt=-1., q=1., tolerance=1E-3, omega=1., outDir="Data", label="Run", initCond="Uniform", method="Jacobi", status=True):
        """
        ...
        # Prepare lattice        
        self.Az = np.zeros((self.xDim, self.yDim))
        self.rho = np.zeros((self.xDim, self.yDim))
        self.approxMid = (self.xDim/2, self.yDim/2)
    ...
    def jacobiUpdate(self):
        ...
        update = np.roll(self.Az, 1, 0) + np.roll(self.Az, -1, 0)\
               + np.roll(self.Az, 1, 1) + np.roll(self.Az, -1, 1)\
               + self.rho
        # Create new lattice of zeros
        newPhi = np.zeros((self.xDim, self.yDim))
        # Update all other than boundaries
        newPhi[1:-1, 1:-1] = update[1:-1, 1:-1]/4. #TODO: check this is /4 for 2D.
        
        # Calculate difference in potentials
        dPhi = np.sum(np.abs(newPhi - self.Az))
        # Update lattice
        self.Az = newPhi
        self.n += 1
        return(dPhi)
    ...
    def analyse(self, showPlot = True):
        ...
        # Get B field
        phiSlice = self.Az
        # B = del x A. Get x and y components (assume dx = dy = 1)
        xField = (np.roll(phiSlice, -1, 0) - np.roll(phiSlice, 1, 0))/(2. * self.dx**2.) # dA_z/dy
        yField = -(np.roll(phiSlice, -1, 1) - np.roll(phiSlice, 1, 1))/(2. * self.dx**2.) # -dA_z/dx
        ...
        for i in range(0, self.xDim):
            for j in range(0, self.yDim):
                ...
                if r > 0.:
                    phiVals.append(self.Az[i,j])
                    rVals.append(r)
                    BVals.append(norm[i,j])
                # Write to file
                outFile.write("{},{},{},{},{}\n".format(i, j, r,self.Az[i,j],norm[i,j]))
        outFile.close()
        # Convert to logarithms
        rVals = np.log10(rVals)
        phiVals = np.log10(phiVals)
        BVals = np.log10(BVals)
        # Fit line in logarithmic space for potential
        ...
        # Perform fit up to log(R) = 1.0 for B field
        params, var = curve_fit(fitFunc, rVals[rVals < 1.0], BVals[rVals < 1.0], p0=p0)
        err = np.sqrt(np.diag(var))
        xFit = np.linspace(min(rVals), max(rVals), 500)
        yFit = fitFunc(xFit, *params)
        pyplot.plot(xFit, yFit, "r--")
        ...
        pyplot.figtext(0.89, 0.84, fitStr, ha="right")
        # Plot phi as a function of radius
        pyplot.plot(rVals, BVals, "b.", ms=0.5)
        pyplot.xlabel(r"Log$_{10}$ R")
        pyplot.ylabel(r"Log$_{10}$ $|B|$")
        pyplot.savefig(self.path + "/RvsB.png", dpi=150)
        if showPlot: pyplot.show()
        else: pyplot.close()
        ...
        pyplot.savefig(self.path + "/B_Field.png")
        if showPlot: pyplot.show()
        else: pyplot.close()
        
    def showCut(self):
        plotSlice = self.Az
        plotLat(plotSlice, self.approxMid[0], self.approxMid[1], "nearest", None, self.path+"/Cut_1.png")
        plotLat(plotSlice, self.approxMid[0], self.approxMid[1], "bilinear", None, self.path+"/Cut_2.png")
        plotLat(plotSlice, self.approxMid[0], self.approxMid[1], "nearest", colors.LogNorm(), self.path+"/Cut_3.png")
        plotLat(plotSlice, self.approxMid[0], self.approxMid[1], "bilinear", colors.LogNorm(), self.path+"/Cut_4.png")
